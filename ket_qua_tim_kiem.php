...
$ngay_ve = isset($_POST['ngay_ve']) ? $_POST['ngay_ve'] : null;
$loai_ve = $_POST['ticket_type'];


// Truy vấn các chuyến tàu khớp với thông tin tìm kiếm
$sql_di = "SELECT * FROM chuyen_tau WHERE ga_di = '$ga_di' AND ga_den = '$ga_den' AND ngay_di = '$ngay_di'";
$result_di = $conn->query($sql_di);

// Truy vấn chuyến về nếu là vé khứ hồi và có ngày về
$result_ve = null;
if ($loai_ve === 'Khứ hồi' && $ngay_ve) {
    $sql_ve = "SELECT * FROM chuyen_tau WHERE ga_di = '$ga_den' AND ga_den = '$ga_di' AND ngay_di = '$ngay_ve'";
    $result_ve = $conn->query($sql_ve);}       
?>
...
<body>
    ...
    (<?php echo isset($_SESSION['cart']) ? count($_SESSION['cart']) : 0; ?>) </a>
        ...
        <div class="train-list">
            ...
            <table>
                ...
                <?php
                        ...
                        if ($result_di->num_rows > 0) {
                            while ($row = $result_di->fetch_assoc()) {
                                echo "<tr>
                                        <td>{$row['id']}</td>
                                        <td>{$row['ten_tau']}</td>
                                        <td>{$row['ga_di']}</td>
            ...
            <!-- Kết quả chuyến về nếu là vé khứ hồi -->
                ...
                <table>
                    ...
                    <?php
                        if ($result_ve && $result_ve->num_rows > 0) {
                            ...
                            echo "<tr><td colspan='8'>Không có chuyến về nào phù hợp.</td></tr>";
                        }
                        $conn->close();
                    ?>
                    </tbody>
